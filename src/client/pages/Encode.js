...
function Encode() {
  ...
  function encode() {
    ...
    if (encryptKey.length > 0) {
      objectMessage = sjcl.encrypt(encryptKey, objectMessage); // encrypting using sjcl(aes)
    } else {
      objectMessage = JSON.stringify({ text: objectMessage });//javascript object to jason string
    }
    ...
    encodeMessage(    // stegnography part in hooks useEncodemessage.js, (color,hash,message)
      imgData.data,
      sjcl.hash.sha256.hash(encryptKey),
      objectMessage
    );
    ctx.putImageData(imgData, 0, 0); //img data will have the modified color of the img
